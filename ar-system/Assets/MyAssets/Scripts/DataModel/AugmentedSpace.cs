...
public class AugmentedSpace : MonoBehaviour
{
    ...
    void Awake()
    {
        // Initially empty or any static ones left
    ...
    void Start()
    {
        if (POIDatabaseFetcher.instance != null)
        {
            POIDatabaseFetcher.instance.POIsLoadedEvent += HandlePOIsLoaded;
            POIDatabaseFetcher.instance.FetchPOIs(augmentation.transform);
        }
    }

    void HandlePOIsLoaded(List<POI> loadedPOIs)
    {
        // Re-grab all POIs including newly spawned ones
        pois = augmentation.GetComponentsInChildren<POI>(true);
        Debug.Log("AugmentedSpace POIs dynamically updated! Total: " + pois.Length);
    }
